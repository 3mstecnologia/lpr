...
def connect_mysql():
    ...
    if conn.open:
        messagebox.showinfo("Alpr", "Connected")
    ...
    try:
        ...
        for plate in results['results']:
            if plate['matches_template']:
                ...
                w = plate['coordinates'][2]['x']-plate['coordinates'][0]['x']
                h = plate['coordinates'][2]['y']-plate['coordinates'][0]['y']

                crop = img[y:y+h, x:x+w]

                cv2.putText(img, plate['plate'], (x, y-5), 0, 1, (0,255,0), 3 )
                cv2.rectangle(img,(x, y), (plate['coordinates'][2]['x'], plate['coordinates'][2]['y']), (0,255,0), 1)
                ...
                cv2image = cv2.cvtColor(crop, cv2.COLOR_BGR2RGBA)
...
mysqlCheckBox = Checkbutton(mysqlFrame, text="Save Plates", variable=saveplate)
